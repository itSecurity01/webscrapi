const { layout, esc, icon, noticeBanner } = require("./layout");

function uploadForm(notice) {
    return `
      ${noticeBanner(notice)}
      <div class="panel">
        <div class="panel-header">${icon("upload")}<h2>1. Upload your product Excel file</h2></div>
        ...
        <form method="post" action="/batch/upload" enctype="multipart/form-data">
          ...
          <div style="margin-top:14px;">
            <button type="submit">${icon("upload")} Upload &amp; check</button>
          </div>
        </form>
      </div>`;
}
...
function sheetPickerPanel(pendingSource, notice) {
    ...
    return `
      ${noticeBanner(notice)}
      <div class="panel">
        <div class="panel-header">${icon("table-columns")}<h2>2. Which sheet(s) do you want to generate from?</h2></div>
        ...
        <form method="post" action="/batch/build" id="sheet-form">
          ...
          <label class="checkbox-line" style="display:inline-flex; margin: 0 0 14px;">
            <input type="checkbox" onchange="document.querySelectorAll('#sheet-form input[name=sheets]:not(:disabled)').forEach(function(c){c.checked=this.checked}, this)"> Select all
          </label>
          <div>
            <button type="submit" onclick="if(!document.querySelector('#sheet-form input[name=sheets]:checked')){alert('Pick at least one sheet.');return false;}">${icon("arrow-right")} Generate input Excel</button>
            <button type="submit" formaction="/batch/upload-reset" class="secondary" formnovalidate>${icon("rotate-left")} Upload a different file instead</button>
          </div>
        </form>
      </div>`;
}
...
function pendingUploadPanel(pendingUpload, pendingSource) {
    ...
    const repickHtml = pendingSource && pendingSource.sheets.length > 1
        ? `<form method="post" action="/batch/repick" style="display:inline;"><button type="submit" class="secondary">${icon("table-columns")} Pick different sheet(s)</button></form>`
        : "";
    ...
    const dupeHtml = dupes.length > 0
        ...
        : `<div class="notice notice-ok">${icon("circle-check")}<div>No duplicate URLs found.</div></div>`;

    return `
      <div class="panel">
        <div class="panel-header">${icon("list-check")}<h2>3. Review &amp; start</h2></div>
        ...
        ${sourceHtml}
        <a class="btn secondary" href="/batch/input-download" onclick="${esc(promptedDownload("/batch/input-download", defaultName))}">${icon("download")} Download generated Excel</a>
      </div>
      ${dupeHtml}
      <div class="panel" style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;">
        <form method="post" action="/batch/start" style="display:inline;">
          <label class="checkbox-line" style="margin-bottom:12px;">
            <input type="checkbox" name="headed">
            Run in headed mode (visible browser window — useful for watching/debugging a run)
          </label>
          <button type="submit">${icon("play")} Start scrape (${distinctUrls} product${distinctUrls === 1 ? "" : "s"})</button>
        </form>
        ${repickHtml}
        <form method="post" action="/batch/upload-reset" style="display:inline;">
          <button type="submit" class="secondary">${icon("rotate-left")} Upload a different file instead</button>
        </form>
      </div>`;
}

const STATE_META = {
    running: { label: "Running", iconName: "spinner", cls: "notice-warn" },
    done: { label: "Done", iconName: "circle-check", cls: "notice-ok" },
    failed: { label: "Failed", iconName: "circle-exclamation", cls: "notice-error" },
};
...
function statusPanel(status, runSummary, notice) {
    ...
    const state = STATE_META[status.state] || { label: status.state, iconName: "circle", cls: "" };

    const logHtml = status.recentLog && status.recentLog.length > 0
        ? `<pre style="background:#101828; color:#d1d5db; padding:14px; border-radius:var(--radius-sm); max-height:280px; overflow:auto; font-size:12px; line-height:1.5; border:1px solid #1f2937;">${esc(status.recentLog.join("\n"))}</pre>`
        : `<p class="muted">No output yet.</p>`;

    const transformHtml = status.transform
        ? status.transform.error
            ? `<div class="notice notice-error">${icon("circle-exclamation")}<div>Auto-transform failed: ${esc(status.transform.error)}</div></div>`
    ...
    const actions = status.state === "running"
        ? `<form method="post" action="/batch/stop" onsubmit="return confirm('Stop the running scrape? Progress so far is safe (state/run.json), but this run won\\'t finish.');">
             <button type="submit" class="secondary">${icon("stop")} Stop scrape</button>
           </form>`
        : `<a class="btn" href="/">${icon("table-list")} Go to the review dashboard</a>
           <form method="post" action="/batch/reset" style="display:inline;">
             <button type="submit" class="secondary">${icon("rotate-left")} Start a new upload</button>
           </form>
           <form method="post" action="/batch/discard" style="display:inline;" onsubmit="return confirm('Discard this test run? Its output/ data and state/run.json entries move into archive/ (nothing is deleted), so these URLs stop counting as already-scraped. Only do this for a throwaway test run \\u2014 not a real batch you still need to review.');">
             <button type="submit" class="danger">${icon("trash")} Discard this test run</button>
           </form>`;

    return `
      ${noticeBanner(notice)}
      <div class="panel">
        <div class="panel-header">${icon(state.iconName, status.state === "running" ? "fa-spin" : "")}<h2>Scrape — ${state.label}</h2></div>
        ...
        ${countsHtml}
        ${status.error ? `<div class="notice notice-error">${icon("circle-exclamation")}<div>Error: ${esc(status.error)}</div></div>` : ""}
        ${transformHtml}
        ${logHtml}
      </div>
      <div class="panel" style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;">${actions}</div>
...
function batchView({ status, pendingSource, pendingUpload, runSummary, notice }) {
    ...
    return layout("Batch", `<p class="back-link"><a href="/">${icon("arrow-left")} back to review dashboard</a></p>${body}`, { active: "batch" });
}
